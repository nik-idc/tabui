...
export class BarElement {
    ...
    constructor(dim, 
    // barCoords: Point,
    bar, showSignature, showTempo, horizontalBarOffset = 0, labelGapHeight = 0) {
        this.dim = dim;
        this.beatElements = [];
        ...
        this.rect = new Rect(horizontalBarOffset, 0, 0, this.dim.tabLineMinHeight + this._labelsGapHeight);
        this.bar = bar;
        this.staffLines = [];
        this.calc();
    }
    ...
    calcBeatsAndRect() {
        ...
        for (let beat of this.bar.beats) {
            const beatElement = new BeatElement(this.dim, beatCoords, beat, this._labelsGapHeight);
     ...
     */
    changeBeatDuration(beat, duration) {
     ...
     */
    changeBarBeats(beatsCount, prevBar) {
     ...
     * @param prevBar Bar preceding this element's bar
     */
    changeBarDuration(duration, prevBar) {
     ...
     */
    get beatsRect() {
        return new Rect(this.timeSigRect.x, this.timeSigRect.y, this.timeSigRect.width, this.timeSigRect.height / 2);
    }
    /**
     * Time signature beats text coords
     */
    get beatsTextCoords() {
        return new Point(this.timeSigRect.x + this.timeSigRect.width / 2, this.timeSigRect.y + this.timeSigRect.height / 3);
    }
    /**
     * Time signature measure text rectangle
     */
    get measureRect() {
        return new Rect(this.timeSigRect.x, this.timeSigRect.y + this.timeSigRect.height / 2, this.timeSigRect.width, this.timeSigRect.height / 2);
    }
    /**
     * Time signature measure text coords
     */
    get measureTextCoords() {
        return new Point(this.timeSigRect.x + this.timeSigRect.width / 2, this.beatsRect.leftBottom.y + this.timeSigRect.height / 3);
    }
    /**
     * Tempo image coords
     */
    get tempoImageRect() {
        return new Rect(this.tempoRect.x, this.tempoRect.y + this.dim.tempoTextSize / 4, this.dim.tempoTextSize, this.dim.tempoTextSize);
    }
    /**
     * Tempo text coords
     */
    get tempoTextCoords() {
        return new Point(this.tempoRect.x + this.dim.tempoTextSize, this.tempoRect.y + this.dim.tempoTextSize);
    }
    /**
     * Bar left border line (array of 2 points)
     */
    get barLeftBorderLine() {
        return [
     ...
     */
    static createBarElement(dim, bar, prevBar, horizontalBarOffset = 0, labelGapHeight = 0) {
        const showSignature = prevBar !== undefined ? bar.signature !== prevBar.signature : true;
        const showTempo = prevBar !== undefined ? bar.tempo !== prevBar.tempo : true;
        const barElement = new BarElement(dim, bar, showSignature, showTempo, horizontalBarOffset, labelGapHeight);
        return barElement;
    }
}
//# sourceMappingURL=bar-element.js.map