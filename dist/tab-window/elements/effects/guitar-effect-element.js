...
export class GuitarEffectElement {
     ...
     */
    constructor(effect, stringNum, noteRect, dim) {
        this.effect = effect;
        ...
        this._startPoint = new Point(this._noteRect.x + this._noteRect.width / 2, this._noteRect.y + this._noteRect.height / 2);
        this.calc();
    }
    /**
     ...
     */
    calcBendPath() {
        const verticalOffset = this._noteRect.height * (this.stringNum - 1) + this._noteRect.height / 2;
        const x = this._startPoint.x;
        const y = this._startPoint.y;
        const curve = this._svgUtils.upCurveSVGHTML(x, y, this._noteRect.width / 2, verticalOffset);
    ...
    calcPrebendPath() {
        ...
        const prebendLine = this._svgUtils.vertLineSVGHTML(prebendLineX, prebendLineY, lineHeight);
        // Step 2: build line arrow
        const lineArrowX = prebendLineX;
        const lineArrowY = prebendLineY - lineHeight;
        const lineArrow = this._svgUtils.verticalArrowSVGHTML(lineArrowX, lineArrowY);
        this._fullHTML = prebendLine + lineArrow;
    }
    /**
     * Build a prebend-and-release path SVG path HTML element
     */
    calcPrebendAndReleasePath() {
        const verticalOffset = this._noteRect.height * (this.stringNum - 1) + this._noteRect.height / 2;
        // Step 1: build line
        const prebendLineX = this._startPoint.x + this._noteRect.width / 4;
        const prebendLineY = this._startPoint.y;
     ...
     */
    calcSlidePath() {
        if (this.effect.options === undefined ||
        ...
        const slideLine = this._svgUtils.lineSVGHTML(slideStartX, slideStartY, slideEndX, slideEndY);
        this._fullHTML = slideLine;
    }
    /**
     * Calc hammer-on or pull-off path
     */
    calcHammerOnOrPullOffPath() {
        ...
        const slideLine = this._svgUtils.horizontalCurveSVGHTML(hpStartX, hpStartY, hpWidth, hpHeight);
    ...
    calcNaturalHarmonicPath() {
        ...
        const nhLine = this._svgUtils.harmonicShapeSVGHTML(nhStartX, nhStartY, nhWidth, nhHeight, false);
    ...
    calcPinchHarmonicPath() {
        ...
        const phLine = this._svgUtils.harmonicShapeSVGHTML(phStartX, phStartY, phWidth, phHeight, true);
...
//# sourceMappingURL=guitar-effect-element.js.map