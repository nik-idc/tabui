...
export class TabWindowHTMLRenderer {
    ...
    _renderedLines;
    _result;
    constructor(tabWindow, assetsPath, container, detailed = false) {
    ...
    renderNoteDetail(beatNotesOffset, noteElement) {
        if (this._detailed) {
    ...
    renderNoteEffects(noteOffset, noteElement) {
    ...
    renderNormalNoteBackground(noteOffset, noteElement) {
        return `
    ...
    renderSelectedNoteBackground(noteOffset, noteElement) {
        return `
      <text x="${noteOffset.x + noteElement.textCoords.x}"
            y="${noteOffset.y + noteElement.textCoords.y}"
            font-size="${this._tabWindow.dim.noteTextSize}px"
    ...
    renderNoteValue(noteOffset, noteElement) {
        if (noteElement.note.fret === undefined) {
            return "";
        }
        const isSelected = this._tabWindow.getSelectedElement() !== undefined &&
    ...
    renderNoteElement(beatNotesOffset, noteElement) {
        const noteOffset = new Point(beatNotesOffset.x, beatNotesOffset.y);
        const renderedNoteDetail = this.renderNoteDetail(beatNotesOffset, noteElement);
    ...
    renderBeatNotesDetail(beatOffset, beatNotesElement) {
        if (!this._detailed) {
            return "";
        }
    ...
    renderNoteElements(beatNotesOffset, beatNotesElement) {
        const renderedNoteElements = [];
        for (const noteElement of beatNotesElement.noteElements) {
            renderedNoteElements.push(this.renderNoteElement(beatNotesOffset, noteElement));
        }
        return renderedNoteElements.join("");
    }
    renderBeatNotesElement(beatOffset, beatNotesElement) {
        const beatNotesOffset = new Point(beatOffset.x + beatNotesElement.rect.x, beatOffset.y + beatNotesElement.rect.y);
        const renderedBeatNotesDetail = this.renderBeatNotesDetail(beatOffset, beatNotesElement);
        const renderedNotes = this.renderNoteElements(beatNotesOffset, beatNotesElement);
        return `
      ${renderedBeatNotesDetail}
      ${renderedNotes}
    `;
    }
    renderEffectLabels(beatOffset, beatElement) {
    ...
    renderBeatDetail(barOffset, beatElement) {
        if (!this._detailed) {
            return "";
        }
        return `
    ...
    renderBeatDuration(beatOffset, beatElement) {
    ...
    renderBeatSelection(barOffset, beatElement) {
    ...
    renderBeatElement(barOffset, beatElement) {
        // Calc offset for each element inside of this beat element
        const beatOffset = new Point(barOffset.x + beatElement.rect.x, barOffset.y + beatElement.rect.y);
        ...
        const renderedBeatNotes = this.renderBeatNotesElement(beatOffset, beatElement.beatNotesElement);
        const renderedSelectionRect = this.renderBeatSelection(barOffset, beatElement);
        return `
    ...
    renderBarStaffLines(barOffset, barElement) {
        const staffLines = [];
    ...
    renderBarSig(barOffset, barElement) {
    ...
    renderBarTempo(barOffset, barElement) {
    ...
    renderBarBeats(barOffset, barElement) {
    ...
    renderBarElement(tleOffset, barElement) {
    ...
    renderTabInfo(showTabCredits = false, showTabName = false, showGuitarInfo = false) {
        const creds = [];
        if (showTabCredits) {
            creds.push(`
    ...
    renderPlayerOverlay() {
        ...
        if (currentBeatElement === undefined) {
            ...
            const beatElementCoords = this._tabWindow.getBeatElementGlobalCoords(currentBeatElement);
            const playerCursorWidth = 5;
            const playerCursorAddHeight = 10;
            cursorRect = new Rect(beatElementCoords.x + currentBeatElement.rect.width / 2, beatElementCoords.y - playerCursorAddHeight, playerCursorWidth, currentBeatElement.rect.height + playerCursorAddHeight);
        }
        return `
    ...
    render(showTabCredits = false, showTabName = false, showGuitarInfo = false) {
        ...
        // Form credentials
        this._credentials = this.renderTabInfo(showTabCredits, showTabName, showGuitarInfo);
        // Player overlay rect
...
//# sourceMappingURL=tab-window-html-renderer.js.map