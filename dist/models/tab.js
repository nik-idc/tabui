...
export class Tab {
     ...
     */
    constructor(id = undefined, name = "Unnamed", artist = "Unknown artist", song = "Unknown song", guitar = new Guitar(), bars = undefined, isPublic = false) {
        this.id = id;
        this.name = name;
        this.artist = artist;
        this.song = song;
    ...
    removeBeat(beatToRemove) {
        ...
        const insideBarIndex = bar.beats.findIndex((beat) => beat.uuid === beatToRemove.uuid);
     ...
     */
    replaceBeats(oldBeats, newBeats) {
     ...
     */
    applyBend(barIndex, beatIndex, stringNum, bendPitch) {
        const guitarNote = this.bars[barIndex].beats[beatIndex].notes[stringNum - 1];
        // Create & apply bend
     ...
     * @returns True if applied, false otherwise
     */
    applyBendAndRelease(barIndex, beatIndex, stringNum, bendPitch, bendReleasePitch) {
        const guitarNote = this.bars[barIndex].beats[beatIndex].notes[stringNum - 1];
     ...
     * @returns True if applied, false otherwise
     */
    applyPrebend(barIndex, beatIndex, stringNum, prebendPitch) {
        const guitarNote = this.bars[barIndex].beats[beatIndex].notes[stringNum - 1];
     ...
     * @returns True if applied, false otherwise
     */
    applyPrebendAndRelease(barIndex, beatIndex, stringNum, prebendPitch, bendReleasePitch) {
        const guitarNote = this.bars[barIndex].beats[beatIndex].notes[stringNum - 1];
        // Create & apply prebend-and-release
        const prebendAndReleaseEffect = new GuitarEffect(GuitarEffectType.PrebendAndRelease, {
            prebendPitch: prebendPitch,
            bendReleasePitch: bendReleasePitch,
        });
     ...
     * @returns True if applied, false otherwise
     */
    applyVibrato(barIndex, beatIndex, stringNum) {
        const guitarNote = this.bars[barIndex].beats[beatIndex].notes[stringNum - 1];
     ...
     * @returns True if applied, false otherwise
     */
    applySlide(barIndex, beatIndex, stringNum) {
        const guitarNote = this.bars[barIndex].beats[beatIndex].notes[stringNum - 1];
        const nextBeat = this.getNextBeat(barIndex, beatIndex);
        const nextNote = nextBeat === undefined ? undefined : nextBeat.notes[stringNum - 1];
        ...
        const slideEffect = new GuitarEffect(GuitarEffectType.Slide, new GuitarEffectOptions(undefined, undefined, undefined, nextHigher));
        // Apply slide
     ...
     * @returns True if applied, false otherwise
     */
    applyHammerOnOrPullOff(barIndex, beatIndex, stringNum) {
        const guitarNote = this.bars[barIndex].beats[beatIndex].notes[stringNum - 1];
        const nextBeat = this.getNextBeat(barIndex, beatIndex);
        const nextNote = nextBeat === undefined ? undefined : nextBeat.notes[stringNum - 1];
        ...
        const hammerOnStartEffect = new GuitarEffect(GuitarEffectType.HammerOnOrPullOff);
     ...
     * @returns True if applied, false otherwise
     */
    applyPinchHarmonic(barIndex, beatIndex, stringNum) {
        const guitarNote = this.bars[barIndex].beats[beatIndex].notes[stringNum - 1];
     ...
     * @returns True if applied, false otherwise
     */
    applyNaturalHarmonic(barIndex, beatIndex, stringNum) {
        const guitarNote = this.bars[barIndex].beats[beatIndex].notes[stringNum - 1];
     ...
     * @returns True if applied, false otherwise
     */
    applyPalmMute(barIndex, beatIndex, stringNum) {
        const guitarNote = this.bars[barIndex].beats[beatIndex].notes[stringNum - 1];
     ...
     */
    applyEffectToNote(barIndex, beatIndex, stringNum, effectType, effectOptions) {
        const note = this.bars[barIndex].beats[beatIndex].notes[stringNum - 1].note;
        if (note.noteValue === NoteValue.None ||
        ...
        switch (effectType) {
            case GuitarEffectType.Bend:
                return this.applyBend(barIndex, beatIndex, stringNum, effectOptions.bendPitch);
            case GuitarEffectType.BendAndRelease:
                return this.applyBendAndRelease(barIndex, beatIndex, stringNum, effectOptions.bendPitch, effectOptions.bendReleasePitch);
            case GuitarEffectType.Prebend:
                return this.applyPrebend(barIndex, beatIndex, stringNum, effectOptions.prebendPitch);
            case GuitarEffectType.PrebendAndRelease:
                return this.applyPrebendAndRelease(barIndex, beatIndex, stringNum, effectOptions.prebendPitch, effectOptions.bendReleasePitch);
    ...
    removeEffectFromNote(barIndex, beatIndex, stringNum, effectIndex) {
     ...
     */
    applyEffectToBeats(beats, effectType, effectOptions) {
        ...
        for (const beat of beats) {
            ...
            for (const note of beat.notes) {
                const applyRes = this.applyEffectToNote(barIndex, beatIndex, note.stringNum, effectType, effectOptions);
                if (!applyRes) {
                    appliedToAll = false;
                }
            }
     ...
     */
    getNextBeat(barIndex, beatIndex) {
     ...
     * @returns Beat (or undefined if can't get next beat)
     */
    getPrevBeat(barIndex, beatIndex) {
        const beatsSeq = this.getBeatsSeq();
        const beatSeqIndex = beatsSeq.indexOf(this.bars[barIndex].beats[beatIndex]);
    ...
    deepCopy() {
        ...
        return new Tab(this.id, this.name, this.artist, this.song, this.guitar, barsCopies, this.isPublic);
    }
    /**
     * Full song name
     */
