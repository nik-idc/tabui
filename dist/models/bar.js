...
export class Bar {
     ...
     */
    constructor(guitar, tempo, beatsCount, duration, beats) {
        this.uuid = randomInt();
        this.guitar = guitar;
        this._tempo = tempo;
    ...
    deepCopy() {
        ...
        return new Bar(this.guitar, this._tempo, this._beatsCount, this.duration, beatsCopies);
    }
    /**
    ...
    set tempo(newTempo) {
        if (newTempo <= 0) {
            throw Error(`${newTempo} is an invalid tempo value: tempo can't be 0 or less`);
        }
    ...
    static fromObject(obj) {
        ...
        obj.beats.forEach((beat) => bar.beats.push(Beat.fromObject(beat)));
        return bar;
    }
    /**
     ...
     */
    static compare(bar1, bar2) {
        if (bar1.guitar !== bar2.guitar ||
