...
export class StaffPlayer<I extends MusicInstrument = MusicInstrument> {
  private _staff: Staff<I>;
  private _currentBeat?: Beat<I>;
  private _synth: Tone.PolySynth;
  private _isPlaying: boolean;
  private _isLooped: boolean;

  constructor(staff: Staff<I>) {
    this._staff = staff;
  ...
  public playFromCurrentBeat(): void {
    ...
    for (let i = currentBeatIndex; i < beatsSeq.length; i++) {
      ...
      if (
        ...
        if (repeatsCount < bar.masterBar.repeatCount! - 1) {
          restartRepeat = true;
          repeatsCount++;
        } else {
          restartRepeat = false;
      ...
      if (
        ...
        leftoverDuration = bpmDurationToSeconds(
          ...
          bar.masterBar.beatsCount * bar.masterBar.duration -
            bar.getActualBarDuration()
        );
      }
    ...
    Tone.getTransport().scheduleOnce((t) => {
      ...
      if (this._isLooped) {
        this.setCurrentBeat(this._staff.getBeatsSeq()[0]);
      } else {
        const firstBeat = this._staff.getBeatsSeq()[0];
        this.stop();
