...
import { Note, NoteJSON, NoteValue } from "../note/note";
import { NoteDuration } from "../note/note-duration";
import { randomInt } from "@/shared";
import {
  BeatGuitarTechniqueType,
  BeatTechniqueType,
  NoteTechniqueType,
  TechniqueType,
} from "../technique/technique-type";
import { BeatTechnique, TechniqueJSON } from "../technique/technique";
import { MusicInstrument } from "../instrument/instrument";
import { Guitar } from "../instrument/guitar/guitar";
import { TupletSettings, TupletSettingsJSON } from "./tuplet-settings";
...
export class Beat<I extends MusicInstrument = MusicInstrument> {
  ...
  /** Beat's techniques */
  private _techniques: BeatTechnique[] = [];
  /** * Base beat duration */
  ...
  /**
   * Adds new technique to the beat
   * @param beatTechnique Beat technique to add
   */
  public addTechnique(beatTechnique: BeatGuitarTechnique): void {
    this._techniques.push(beatTechnique);
  }

  /**
   * Removes technique from the beat
   * @param type Technique type
   */
  public removeTechnique(type: BeatGuitarTechniqueType): void {
   ...
   */
  public hasNoteTechnique(type: NoteTechniqueType): boolean {
    return this._notes.some((n) => n.hasTechnique(type));
  }

  /**
   * Checks if the beat has a technique
   * @param type Type of beat technique
   * @returns True if beat has technique, false otherwise
   */
  public hasBeatTechnique(type: BeatTechniqueType): boolean {
    return this._techniques.some((t) => t.type === type);
  }
  ...
  public deepCopy(): Beat<I> {
    ...
    for (let i = 0; i < this._techniques.length; i++) {
      techniques[i] = this._techniques[i].deepCopy();
    }
  ...
  public toJSON(): BeatJSON {
    ...
    const techniquesJSON = [];
    for (const technique of this._techniques) {
      techniquesJSON.push(technique.toJSON());
    }
  ...
  /** Techniques getter */
  public get techniques(): BeatTechnique[] {
