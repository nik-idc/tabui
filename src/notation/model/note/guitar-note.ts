...
import { BendTechniqueOptions } from "../technique/guitar-technique/note-guitar-technique/bend-options";
import {
  NoteGuitarTechnique,
  NOTE_TECHNIQUES_INCOMPATIBILITY,
  NoteGuitarTechniqueJSON,
} from "../technique";
import { NoteGuitarTechniqueType } from "../technique/technique-type";
...
export interface GuitarNoteJSON {
  ...
  stringNum: number;
  fret: number | null;
  techniques: NoteGuitarTechniqueJSON[];
}
...
export class GuitarNote implements Note<Guitar> {
  ...
  private _techniques: NoteGuitarTechnique[];
  ...
  constructor(
    ...
    techniques: NoteGuitarTechnique[] = []
  ) {
    this.uuid = randomInt();
    this.beat = beat;
   ...
   */
  public addTechnique(guitarTechnique: NoteGuitarTechnique): boolean {
   ...
   */
  public removeTechnique(type: NoteGuitarTechniqueType): void {
   ...
   */
  public techniqueApplicable(type: NoteGuitarTechniqueType): boolean {
    return !this._techniques.some((e) =>
      NOTE_TECHNIQUES_INCOMPATIBILITY[e.type].includes(type)
    );
  }
   ...
   */
  public hasTechnique(type: NoteGuitarTechniqueType): boolean {
  ...
  public toJSON(): GuitarNoteJSON | null {
    ...
    const fxJSON: NoteGuitarTechniqueJSON[] = [];
    for (const technique of this._techniques) {
  ...
  /** Techniques array */
  public get techniques(): NoteGuitarTechnique[] {
